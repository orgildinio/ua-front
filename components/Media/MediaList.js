...
import { getMedias, getNews } from "lib/getFetchers";

import BlockLoad from "../General/BlockLoad";
import NotFound from "../General/Notfound";
import MediaItem from "./MediaItem";

const MediaList = ({ plusQuery = "plus=none" }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const { medias, pagination } = await getMedias(`${qry}&${plusQuery}`);
            if (medias) setData(medias);
    ...
    useEffect(() => {
        const fetchData = async (query) => {
            setLoading(true);
            const { medias, pagination } = await getMedias(
                query + "&" + plusQuery
            );
